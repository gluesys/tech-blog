...
SPDK 에서 제공하는 모든 기능들은 rpc[^3]로 통신하며, 요청과 응답은 JSON 포맷을 사용합니다.  이는 커널에서 시스템 콜을 통해 장치 드라이버에 사용자 명령을 전달하는 것과 같은 구조입니다. 즉, SPDK 드라이버를 사용자 공간의 특정 프로그램으로 실행시킨 상태에서, rpc 통신을 통해 사용자 명령을 SPDK 드라이버(프로세스)에 전달합니다.

더 깊게 들어가지 말고 나중에 심화 포스팅에서 다루겠습니다. 필요하신 분들은 [링크1](https://spdk.io/doc/app_overview.html) [링크2](https://spdk.io/doc/jsonrpc.html)를 참고하시면 됩니다.

리눅스에서는 rpc 통신을 위해서 소켓을 오픈합니다. 따라서 rpc 통신을 위해 소켓을 생성해야 합니다. 동작 여부를 확인하기 위해 소켓 프로세스는 포그라운드로 실행하겠습니다. 별도의 터미널을 띄우고 `build/bin/spdk_tgt`를 실행합니다. 
