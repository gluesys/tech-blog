...
안녕하세요. 이번 포스트에서는 NVMe SSD와 SPDK의 최신 코드를 이용해서 테스트 환경을 구축해 보고 SPDK에서 제공하는 스토리지 기능들을 실습해 보겠습니다.
지난 [포스팅](https://tech.gluesys.com/blog/2022/02/18/SPDK_1.html)에서 소개한 바와 같이 SPDK는 사용자 공간(userspace) 계층에서 NVMe와 같은 초고속 SSD 장치를 효율적으로 사용하기 위한 프레임워크입니다.
...
SPDK는 크게 스토리지 전용의 인터페이스(Storage Protocol)과 SPDK의 핵심 개념이 정의된 디바이스 드라이버(Driver), 그리고 커널에서 제공하던 블록 장치 기술(Storage Service)로 구성됩니다. SPDK에서는 블록 장치 기술을 'Block Device Drvier' 또는 'Block Device Module', 줄여서 'bdev'로 표현합니다.
