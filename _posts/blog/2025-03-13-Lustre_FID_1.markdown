...
# 들어가기 앞서
이번 블로그에서 설명할 내용은 Lustre를 활용하고 있는 환경에서 사용자가 새로운 파일을 생성하면, 내부적으로 이루어지는 과정을 분석한 내용을 담고 있습니다
 본 내용은 Lustre 전반적인 구조에 대해 기본적으로 알고 있는 것으로 가정하고 기술하고 있어서, Lustre의 구조가 궁금하신 분들은 `Lustre 파일시스템과 GPUDirect Storage 소개`\[1\] 글을 참고하실 수 있습니다.
<br> 또한 Lustre에서 새로운 파일의 생성은 다양한 Component들의 상호작용에 의해 이루어지기 때문에, 총 2편에 걸쳐 설명할 예정입니다.
...
`Layout EA`는 `Layout Extended Attributes`의 약자로, Lustre에서 파일의 레이아웃을 관리하기 위한 자료구조 입니다. 이 자료구조는 파일을 식별하기 위한 `FID(File Identifiers)` 정보와
